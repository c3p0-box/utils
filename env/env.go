// Package env provides helpers to populate configuration structs from OS
// environment variables using struct tags. It does not parse config files;
// only environment variables are supported.
package env
...
// ReadEnv reads OS environment variables into the provided structure based on `env` tags.
//
// Supported tags:
//   - `env`: comma-separated list of environment variable names to read (first found wins)
//   - `env-default`: default value used if no environment variables are set and the field is zero
...
// UpdateEnv re-reads OS environment variables into the provided structure.
// Only fields marked with the `env-upd` tag will be updated.
